...
pub contract SportsCastCollectible_NFT: NonFungibleToken {

    // SportsCastCollectible_NFT Events
    //
    // Emitted when the SportsCastCollectible_NFT contract is created
    ...
    // totalSupply
    // The total number of SportsCastCollectible_NFT that have been minted
    //
    ...
    // A Series is special resource type that contains functions to mint SportsCastCollectible_NFT NFTs, 
	...
	pub resource Series {
        ...
        init(
            ...
            SportsCastCollectible_NFT.seriesData[seriesId] = SeriesData(
                    seriesId: seriesId,
                    metadata: metadata
            )
        ...
        pub fun addNftSet(
            ...
            SportsCastCollectible_NFT.setData[setId] = newNFTSet
        ...
        pub fun updateSeriesMetadata(metadata: {String: String}) {
            ...
            SportsCastCollectible_NFT.seriesData[self.seriesId] = newSeriesMetadata
	    ...
	    pub fun mintSportsCastCollectible_NFT(
            ...
            pre {
                ...
                self.numberEditionsMintedPerSet[setId]! <= SportsCastCollectible_NFT.getSetMaxEditions(setId: setId)!:
			...
			recipient.deposit(token: <-create SportsCastCollectible_NFT.NFT(
                tokenId: tokenId,
                setId: setId,
		...
		pub fun batchMintSportsCastCollectible_NFT(
            ...
            for tokenId in tokenIds {
                self.mintSportsCastCollectible_NFT(
                    recipient: recipient,
                    tokenId: tokenId,
                    setId: setId
                )
    ...
    // A resource that represents the SportsCastCollectible_NFT NFT
    //
    pub resource NFT: NonFungibleToken.INFT {
        ...
        init(
          ...
          editionNum: UInt32) {
            ...
            let seriesId = SportsCastCollectible_NFT.getSetSeriesId(setId: setId)!
    ...
    pub resource Admin {
        ...
        pub fun addSeries(seriesId: UInt32, metadata: {String: String}) {
            pre {
                SportsCastCollectible_NFT.series[seriesId] == nil:
            ...
            SportsCastCollectible_NFT.series[seriesId] <-! newSeries
        }
        ...
        pub fun borrowSeries(seriesId: UInt32): &Series  {
            ...
            return (&SportsCastCollectible_NFT.series[seriesId] as &Series?)!
        }
    ...
    // to allow others to deposit SportsCastCollectible_NFT into their Collection. It also allows for reading
    // the details of SportsCastCollectible_NFT in the Collection.
    pub resource interface SportsCastCollectible_NFTCollectionPublic {
        pub fun deposit(token: @NonFungibleToken.NFT)
        pub fun batchDeposit(tokens: @NonFungibleToken.Collection)
        pub fun getIDs(): [UInt64]
        pub fun borrowNFT(id: UInt64): &NonFungibleToken.NFT
        pub fun borrowSportsCastCollectible_NFT(id: UInt64): &SportsCastCollectible_NFT.NFT? {
            // If the result isn't nil, the id of the returned reference
            // should be the same as the argument to the function
            post {
                (result == nil) || (result?.id == id):
                    "Cannot borrow SportsCastCollectible_NFT reference: The ID of the returned reference is incorrect"
            }
        }
    }

    // Collection
    // A collection of SportsCastCollectible_NFT NFTs owned by an account
    //
    pub resource Collection: SportsCastCollectible_NFTCollectionPublic, NonFungibleToken.Provider, NonFungibleToken.Receiver, NonFungibleToken.CollectionPublic {
        ...
        //
        pub fun deposit(token: @NonFungibleToken.NFT) {
            let token <- token as! @SportsCastCollectible_NFT.NFT
        ...
        // borrowSportsCastCollectible_NFT
        // Gets a reference to an NFT in the collection as a SportsCastCollectible_NFT,
        // exposing all of its fields.
        // This is safe as there are no functions that can be called on the SportsCastCollectible_NFT.
        //
        pub fun borrowSportsCastCollectible_NFT(id: UInt64): &SportsCastCollectible_NFT.NFT? {
            let ref = &self.ownedNFTs[id] as auth &NonFungibleToken.NFT?
            return ref as! &SportsCastCollectible_NFT.NFT?
        }
    ...
    // fetch
    // Get a reference to a SportsCastCollectible_NFT from an account's Collection, if available.
    // If an account does not have a SportsCastCollectible_NFT.Collection, panic.
    ...
    //
    pub fun fetch(_ from: Address, id: UInt64): &SportsCastCollectible_NFT.NFT? {
        let collection = getAccount(from)
            .getCapability(SportsCastCollectible_NFT.CollectionPublicPath)
            .borrow<&SportsCastCollectible_NFT.Collection{SportsCastCollectible_NFT.SportsCastCollectible_NFTCollectionPublic}>()
            ?? panic("Couldn't get collection")
        // We trust SportsCastCollectible_NFT.Collection.borrowSportsCastCollectible_NFT to get the correct id
        // (it checks it before returning it).
        return collection.borrowSportsCastCollectible_NFT(id: id)
    }

    // getAllSeries returns all the sets
    //
    // Returns: An array of all the series that have been created
    pub fun getAllSeries(): [SportsCastCollectible_NFT.SeriesData] {
        return SportsCastCollectible_NFT.seriesData.values
    }

    // getAllSets returns all the sets
    //
    // Returns: An array of all the sets that have been created
    pub fun getAllSets(): [SportsCastCollectible_NFT.NFTSetData] {
        return SportsCastCollectible_NFT.setData.values
    }
    ...
    pub fun getSeriesMetadata(seriesId: UInt32): {String: String}? {
        return SportsCastCollectible_NFT.seriesData[seriesId]?.getMetadata()
    }
    ...
    pub fun getSetMaxEditions(setId: UInt32): UInt32? {
        return SportsCastCollectible_NFT.setData[setId]?.maxEditions
    }
    ...
    pub fun getSetMetadata(setId: UInt32): {String: String}? {
        return SportsCastCollectible_NFT.setData[setId]?.getMetadata()
    }
    ...
    pub fun getSetSeriesId(setId: UInt32): UInt32? {
        return SportsCastCollectible_NFT.setData[setId]?.seriesId
    }
    ...
    pub fun getIpfsMetadataHashByNftEdition(setId: UInt32, editionNum: UInt32): String? {
        ...
        if let set = SportsCastCollectible_NFT.setData[setId] {
	...
	init() {
        // Set named paths
        self.CollectionStoragePath = /storage/SportsCastCollectible_NFTCollection
        self.CollectionPublicPath = /public/SportsCastCollectible_NFTCollection
        self.AdminStoragePath = /storage/SportsCastCollectible_NFTAdmin
        self.AdminPrivatePath = /private/SportsCastCollectible_NFTAdminUpgrade
        ...
        self.account.link<&SportsCastCollectible_NFT.Admin>(
